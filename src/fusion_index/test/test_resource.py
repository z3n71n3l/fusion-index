...
def PUT(self, agent, path, data):
    ...
    return self.successResultOf(
        agent.request(b"PUT", path, bodyProducer=FileBodyProducer(StringIO(data)))
    )
...
class LookupAPITests(SynchronousTestCase):
    ...
    def assertLookupLogging(self, logger):
        ...
        assertContainsFields(
            self,
            put.start_message,
            {"environment": u"someenv", "indexType": u"sometype", "key": u"somekey"},
        )
    ...
    def test_storeAndRetrieve(self, logger):
        ...
        response = PUT(self, agent, b"/lookup/someenv/sometype/somekey", b"data")
    ...
    def test_storeOverwrite(self):
        ...
        response = PUT(self, agent, b"/lookup/someenv/sometype/somekey", b"newdata")
        self.assertEqual(response.code, http.NO_CONTENT)
    ...
    def test_storeMultiple(self):
        ...
        self.assertEqual(GET(self, agent, b"/lookup/e2/t1/k1").code, http.NOT_FOUND)
        self.assertEqual(GET(self, agent, b"/lookup/e1/t2/k1").code, http.NOT_FOUND)
        self.assertEqual(GET(self, agent, b"/lookup/e1/t1/k2").code, http.NOT_FOUND)
        self.assertEqual(GET(self, agent, b"/lookup/e1/t2/k2").code, http.NOT_FOUND)
        self.assertEqual(GET(self, agent, b"/lookup/e2/t1/k2").code, http.NOT_FOUND)
        self.assertEqual(GET(self, agent, b"/lookup/e2/t2/k1").code, http.NOT_FOUND)
...
class SearchAPITests(SynchronousTestCase):
    ...
    def assertSearchLogging(self, logger):
        ...
        assertContainsFields(self, put.end_message, {"searchValue": u"somevalue"})
        self.assertTrue(put.succeeded)
    ...
    def test_storeRetrieveDelete(self, logger):
        ...
        self.assertEqual(response.code, http.OK)
        self.assertEqual(
            response.headers.getRawHeaders("Content-Type"), ["application/json"]
        )
        self.assertEqual(json.loads(data(self, response)), [u"result"])

        response = DELETE(
            self, agent, b"/search/exact/someenv/someindex/entries/result/type"
        )
        self.assertEqual(response.code, http.NO_CONTENT)
        ...
        self.assertEqual(json.loads(data(self, response)), [])
    ...
    def test_storeMultiple(self):
        ...
        response = GET(self, agent, b"/search/exact/e/i/results/value")
        self.assertEqual(response.code, http.OK)
        self.assertEqual(
            response.headers.getRawHeaders("Content-Type"), ["application/json"]
        )
        self.assertEqual(set(json.loads(data(self, response))), {"result1", "result2"})

        response = GET(self, agent, b"/search/exact/e/i/results/value/type2")
        self.assertEqual(response.code, http.OK)
        self.assertEqual(
            response.headers.getRawHeaders("Content-Type"), ["application/json"]
        )
        self.assertEqual(json.loads(data(self, response)), ["result2"])
    ...
    def test_insertTwice(self):
        ...
        """
        agent = ResourceTraversalAgent(self._resource())
        response = PUT(self, agent, b"/search/exact/e/i/entries/result/type", b"value")
        self.assertEqual(response.code, http.NO_CONTENT)
        response = PUT(self, agent, b"/search/exact/e/i/entries/result/type", b"value")
        self.assertEqual(response.code, http.NO_CONTENT)
        ...
        self.assertEqual(set(json.loads(data(self, response))), {"result"})
    ...
    def assertSearchLogging2(self, logger):
        ...
        assertContainsFields(self, put1.end_message, {"searchValue": u"value"})
        ...
        assertContainsFields(self, put2.end_message, {"searchValue": u"value"})
    ...
    def test_exactAndPrefix(self, logger):
        ...
        self.assertEqual(json.loads(data(self, response)), ["result1"])

        response = GET(self, agent, b"/search/prefix/e/i/results/va")
        self.assertEqual(response.code, http.OK)
        self.assertEqual(
            response.headers.getRawHeaders("Content-Type"), ["application/json"]
        )
    ...
    def test_invalidSearchClass(self):
        ...
        """
        agent = ResourceTraversalAgent(self._resource())
        response = GET(self, agent, b"/search/invalid/e/i/value/")
        self.assertEqual(response.code, http.NOT_FOUND)
